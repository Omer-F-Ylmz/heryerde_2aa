...
public sealed partial class PiiMaskEnricher : ILogEventEnricher
{
    ...
    /// <summary>Alan adı telefon/e-posta/adres işaret ediyor mu.</summary>
    public static bool IsPiiName(string name) => PiiNames.Any(pii => name.Contains(pii, StringComparison.OrdinalIgnoreCase));

    /// <summary>Metindeki e-posta ve telefon desenlerini "***" yapar.</summary>
    public static string MaskText(string text) => PiiPattern().Replace(text, Mask);
